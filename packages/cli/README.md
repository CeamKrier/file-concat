...
- Test files are excluded by each ecosystem's naming convention, matched on the file name at any depth: `*.test.ts` / `*.spec.js` and their siblings, `*_test.go`, `test_*.py` and `*_test.py`, `conftest.py`, `*_spec.rb`, `*Test.java`, `*Tests.cs`, `*Test.php`, `*Tests.swift`. This is naming rather than detection, so a `tests/` directory whose files carry no convention still comes through, as do Rust's `tests/*.rs`. A `__tests__` directory is not ignored for its name alone. Add `--exclude` patterns for anything your project spells differently.
