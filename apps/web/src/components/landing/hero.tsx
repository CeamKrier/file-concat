...
async function readEntries(files: Array<{ file: File; path: string }>): Promise<StagedEntry[]> {
  const out: StagedEntry[] = [];
  for (const { file, path } of files) {
    try {
      out.push({ file, path, content: await file.text() });
    } catch {
      // skip unreadable
    }
  }
...
async function readFileList(fileList: FileList): Promise<StagedEntry[]> {
  const out: StagedEntry[] = [];
...
export function Hero() {
  ...
  const handleDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      try {
        const { collected } = await collectFromDataTransfer(e.dataTransfer.items);
        finishCollection(await readEntries(collected));
      } catch {
        setState({
          kind: "error",
  ...
  const handleFileInput = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setState({ kind: "processing", message: "Reading files…" });
      const collected = await readFileList(e.target.files);
      e.target.value = "";
      finishCollection(collected);
    },
    [finishCollection],
  );
