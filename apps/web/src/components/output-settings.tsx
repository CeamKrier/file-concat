        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex flex-1 flex-wrap gap-2">
            {QUICK_SIZES.map((size) => (
              <Button
                key={size.value}
                variant={maxFileSize === size.value ? "secondary" : "outline"}
                size="sm"
                onClick={() => setMaxFileSize(size.value)}
                disabled={disabled}
                className="h-7 flex-1 px-3 py-1 text-sm"
              >
                {size.label}
              </Button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="icon"
              onClick={() => adjustMaxFileSize(false)}
              disabled={disabled || maxFileSize <= 32}
            >
            ...
            <Button
              variant="outline"
              size="icon"
              onClick={() => adjustMaxFileSize(true)}
              disabled={disabled}
            >
