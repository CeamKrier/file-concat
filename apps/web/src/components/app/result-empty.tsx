...
/**
 * The novice rescue. Reached when nothing combinable was found (e.g. only
 * images). Never a dead end: explains what happened and offers the next step.
 */
export function ResultEmpty({ droppedFiles, onStartOver }: ResultEmptyProps) {
  ...
  return (
    <section className="animate-fade-up mx-auto w-full max-w-[560px] px-4 pt-12 text-center motion-reduce:animate-none">
      <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border-2 border-[oklch(var(--info)/0.4)] bg-[oklch(var(--info)/0.12)]">
        <ImageOff className="text-info h-6 w-6" strokeWidth={2} />
      </span>

      <h2 className="font-display text-ink mt-4 text-2xl font-bold tracking-[-0.02em]">
        These look like images, not text
      </h2>
      <p className="text-ink-secondary mx-auto mt-2 max-w-[440px] text-[15px] leading-relaxed">
        FileConcat bundles text (code, docs, configs and data) into one document. Images and
        binaries can&apos;t be combined this way, so nothing was left to pack.
      </p>
