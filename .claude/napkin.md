...
- `minimatch(path, "src/**/*", { matchBase: true })` does NOT match `"myrepo/src/app.tsx"` — `matchBase` is ignored when the pattern contains `/`, and the slashed pattern needs the path to *start with* `src/`. If users type include/ignore patterns expecting "anywhere in the path", the current `apps/web/src/app.tsx` `isExcludedPath` will silently drop everything. This is a known live bug in the codebase as of 2026-05-17, recorded for future fix work; the immediate UX patch (filter-drop alert) at `app.tsx:1058` surfaces it but does not solve the matching semantics.
